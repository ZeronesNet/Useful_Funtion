# 模块注释
# Check_File函数通过传入FilesList,FileType筛选特定的文件
# Check_Name函数通过传入FileList,TarFile_Name实现文件搜索功能

def Check_File(FilesList,FileType):
    # 传入一个文件路径的列表
    Files_Check = []
    ...
    for SingleFile in FilesList:
        ...
        # 提取所有可能的后缀并传入File_end中
        for e in File_end:
            for e1 in FileType:
                if e == e1:
                    Files_Check.append(SingleFile)
        # 取得单个后缀，与FileType中的后缀进行对比
    return(Files_Check)

def Check_Name(FileList,TarFile_Name):
